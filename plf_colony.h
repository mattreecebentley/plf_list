...
private:
	...
	inline void blank() PLF_COLONY_NOEXCEPT
	{
			...
			if (std::is_standard_layout<colony>::value && std::is_trivial<group_pointer_type>::value && std::is_trivial<aligned_pointer_type>::value && std::is_trivial<skipfield_pointer_type>::value && NULL == reinterpret_cast<void *>(0)) // if all pointer types are trivial, and NULL is (almost always) zero, we can just nuke it from orbit with memset:
			{
